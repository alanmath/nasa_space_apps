        ...
        for ev in pygame.event.get() :
            ...
            if ev.type == pygame.KEYDOWN:  # Verifica se o usuario apertou alguma tecla e então altera o estado de movimentacao dele ou o estado de interacao com os itens da tela. 
                ...
                for item in self.itens:
                    ...
                    if ev.key == pygame.K_SPACE and state['jogador'].pode_abrir(item) and (item.status == False) and not state['jogador'].mao: # Verifica se o jogador pode interagir com intens.
                           
                        item.status = True
                        state['jogador'].mao = True
                        pygame.mixer.Sound.play(assets['interacao']) # Efeito sonoro das interacoes
...
class Nave :
    ...
    def __init__(self, img):  
        ...
        self.ponto = (960,540)
        self.larg, self.alt = self.img[0].get_size()
        self.rect = pygame.Rect(self.ponto, (self.larg, self.alt))
        self.status = 0
...
class Player(pygame.sprite.Sprite) :
    ...
    def anda (self):
        ...
        if self.pos_x >= 0 and self.pos_x <= 1920 and self.pos_y >= 0 and self.pos_y <= 1010:
        ...
        #For itens em state.itens_moveis :
        #  If itme.status== True :
        #  Item.x =  self.pos_x 
        #  Item.y =  self.pos_y 
